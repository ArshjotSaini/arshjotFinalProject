...
func Mapinit(path string) {
	if !IsFileExist(path) {
		...
		for _, job := range details {
			...
			// fmt.Println(loc)
			ShowMap([][]string{{strconv.FormatFloat(loc.Lat, 'f', -1, 64)}, {strconv.FormatFloat(loc.Lng, 'f', -1, 64)}}, loc, path)
		}
	}
}

func CreateMap(locationList []string, path string) {
	LocationCache = make(map[string]*geo.Location)
	JobCounts = make(map[string]int)
	var loc *geo.Location
	for _, job := range locationList {
		loc = FindLocation(job)
		ShowMap([][]string{{strconv.FormatFloat(loc.Lat, 'f', -1, 64)}, {strconv.FormatFloat(loc.Lng, 'f', -1, 64)}}, loc, path)
	}
}
...
func ShowMap(data [][]string, loc *geo.Location, path string) {
	...
	ThrowError(err)
	if err := gg.SavePNG(path, image); err != nil {
		panic(err)
	}
